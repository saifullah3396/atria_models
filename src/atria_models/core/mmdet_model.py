    ...
    def _build(self, **kwargs) -> "BaseDetector":
        ...
        for path in self._resolved_model_search_paths:
            if (path / self.config.model_name).exists():
                cfg = Config.fromfile(path / self.config.model_name)
        ...
        raise FileNotFoundError(
            f"Model {self.config.model_name} not found in search paths {[str(x) for x in self._resolved_model_search_paths]}"
        )
